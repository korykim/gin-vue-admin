  ...
  <div>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="demo_ToyStore" />
            <ExportExcel  template-id="demo_ToyStore" filterDeleted/>
            <ImportExcel  template-id="demo_ToyStore" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt"width="180">
            ...
            <el-table-column sortable align="left" label="店名" prop="name" width="120" />

            <el-table-column align="left" label="地址" prop="address" width="120" />

            <el-table-column align="left" label="电话" prop="phone" width="120" />

            <el-table-column align="left" label="营业时间" prop="openHours" width="120" />

            <el-table-column align="left" label="店主" prop="owner" width="120" />
    ...
    <template #default="scope">
      <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.logo)" fit="cover"/>
            ...
            <el-table-column align="left" label="管理员" prop="manager" width="120">
    <template #default="scope">
        <el-tag v-for="(item,key) in filterDataSource(dataSource.manager,scope.row.manager)" :key="key" style="margin-right: 1px;">
             {{ item }}
        </el-tag>
    </template>
</el-table-column>
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateToyStoreFunc(scope.row)">编辑</el-button>
    ...
    <el-image style="width: 50px; height: 50px" :preview-src-list="returnArrImg(detailFrom.logo)" :src="getUrl(detailFrom.logo)" fit="cover" />
</el-descriptions-item>
                    <el-descriptions-item label="管理员">
    <template #default="scope">
        <el-tag v-for="(item,key) in filterDataSource(dataSource.manager,detailFrom.manager)" :key="key" style="margin-right: 1px;">
             {{ item }}
        </el-tag>
    </template>
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getToyStoreList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteToyStoreByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateToyStoreFunc = async(row) => {
    const res = await findToyStore({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteToyStoreFunc = async (row) => {
    const res = await deleteToyStore({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
