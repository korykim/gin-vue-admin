  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt"width="180">
            ...
            <el-table-column sortable align="left" label="姓名" prop="name" width="120" />
            ...
            <el-table-column sortable align="left" label="入职日期" prop="startDate" width="180">
   <template #default="scope">{{ formatDate(scope.row.startDate) }}</template>
</el-table-column>
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateEmployeeProfileFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getEmployeeProfileList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteEmployeeProfileByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateEmployeeProfileFunc = async(row) => {
    const res = await findEmployeeProfile({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteEmployeeProfileFunc = async (row) => {
    const res = await deleteEmployeeProfile({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
