...
<template>
  ...
  <el-input v-model="searchInfo.name" placeholder="搜索条件" />
    ...
    {{ filterDict(scope.row.sex,genderOptions) }}
            ...
            <el-table-column label="简介" prop="profile" width="200">
    <template #default="scope">
         <div class="file-list">
           <el-tag v-for="file in scope.row.profile" :key="file.uid" @click="onDownloadFile(file.url)">{{ file.name }}</el-tag>
         </div>
    </template>
</el-table-column>
            <el-table-column label="头像" prop="picture" width="200">
    <template #default="scope">
      <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.picture)" fit="cover"/>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateStudentFunc(scope.row)">编辑</el-button>
    ...
    <div class="fileBtn" v-for="(item,index) in detailFrom.profile" :key="index">
