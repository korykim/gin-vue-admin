<template>
  <div>
    ...
    <template #default="scope">
      <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.avatar)" fit="cover"/>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateQmUserFunc(scope.row)">编辑</el-button>
            <el-button  type="primary" link icon="key" class="table-button" @click="resetPasswordFunc(scope.row)">重置密码</el-button>
    ...
    <el-input v-model="formData.username" :clearable="false" placeholder="请输入用户名" :disabled="type === 'update'" />
</el-form-item>
...
const getTableData = async() => {
  const table = await getQmUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteQmUserByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateQmUserFunc = async(row) => {
    const res = await findQmUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteQmUserFunc = async (row) => {
    const res = await deleteQmUser({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const submitResetPassword = async () => {
  passwordFormRef.value?.validate(async (valid) => {
    if (valid) {
      const res = await adminChangePassword(passwordForm.value)
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '密码重置成功'
