...
service.interceptors.request.use(
  ...
  error => {
    if (!error.config && !error.config.donNotShowLoading) {
      closeLoading()
    }
...
service.interceptors.response.use(
  response => {
    ...
    if (response.headers['new-token']) {
      ...
      console.log('收到新token:', newToken)
      userStore.setToken(newToken)
    }
    
    // 处理 set-cookie 头
    const setCookie = response.headers['set-cookie']
    if (setCookie && setCookie.includes('x-token=')) {
      console.log('收到set-cookie:', setCookie)
      // 注意：浏览器会自动处理 set-cookie 头，这里不需要手动处理
    }
